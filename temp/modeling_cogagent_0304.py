...
def attention_fn(query_layer, key_layer, value_layer, attention_mask=None, scaling_attention_score=True, attention_dropout=None):
    # Compute the attention scores
    attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
    
    if scaling_attention_score:
        scale = math.sqrt(query_layer.size(-1))
    ...
    # Softmax to normalize the scores to probabilities
    attention_probs = torch.nn.functional.softmax(attention_scores, dim=-1)
    
    # Optionally apply dropout to attention probabilities
    if attention_dropout is not None:
        attention_probs = torch.nn.functional.dropout(attention_probs, p=attention_dropout)
    
    # Multiply the normalized scores by the value layer
    context_layer = torch.matmul(attention_probs, value_layer)
    
    # Return both the context layer and the attention scores (probabilities)
...
class VisionExpertAttention(nn.Module):
    ...
    def forward(
        ...
        context_layer, attention_scores = attention_fn(
            query_layer=query_states, key_layer=key_states, value_layer=value_states, 
            attention_mask=attention_mask, scaling_attention_score=True, attention_dropout=None
        )
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is None:
            mixed_key_value_layer = self.key_value(encoder_outputs)
            mixed_key_layer, mixed_value_layer = torch.split(mixed_key_value_layer, self.cross_compute_hidden_size, dim=-1)
        else:
            mixed_key_layer, mixed_value_layer = past_key_value
        ...
        # Assuming you have an attention computation function here, e.g., torch.matmul for simplicity
        ...
        context_layer = context_layer.view(bsz, q_len, self.cross_compute_hidden_size)
        ...
        return attn_output, attention_scores, (key_states, value_states) if use_cache else None
...
class CogAgentDecoderLayer(nn.Module):
    ...
    def forward(
            ...
            extract_intermediate_representation: Optional[bool] = False, # added
    ) -> Tuple[torch.FloatTensor, Optional[Tuple[torch.Tensor, torch.Tensor]], Optional[Dict[str, torch.Tensor]]]:
        ...
        # Self Attention
        attn_output, self_attn_weights, present_key_value_self = self.self_attn(
        ...
        # Cross Attention with Encoder Outputs if provided
        if encoder_outputs is not None:
            attention_output, cross_attn_weights, present_key_value_cross = self.cross_attn(
            ...
            present_key_value_cross = (None, None)  # Handle case where no cross attention occurs

        mlp_input = self.post_attention_layernorm(hidden_states)
        mlp_output = self.mlp(mlp_input, token_type_ids=token_type_ids)
        hidden_states = mlp_output + hidden_states

        # Combine self and cross attention past key values
        present_key_value = present_key_value_self + present_key_value_cross

        outputs = (hidden_states, present_key_value)

        if output_attentions:
            outputs += (self_attn_weights, cross_attn_weights if encoder_outputs is not None else None)

        if extract_intermediate_representation:
            # Capture intermediate representations as needed
            intermediate_representations["self_attn_weights"] = self_attn_weights if output_attentions else None
            intermediate_representations["cross_attn_weights"] = cross_attn_weights if output_attentions and encoder_outputs is not None else None
            outputs += (intermediate_representations,)
...
class RepBaseModel(BaseModelOutputWithPast):
    def __init__(self, intermediate_representations: Optional[Dict[str, torch.Tensor]] = None, **kwargs):
        super().__init__(**kwargs)
        self.intermediate_representations = intermediate_representations
...
class CogAgentModel(CogAgentPreTrainedModel):
    ...
    def forward(
        ...
        return self.llm_forward(
            ...
            extract_intermediate_representation=extract_intermediate_representation, # added
        )
    ...
    def llm_forward(
        ...
        intermediate_representations_aggregated = {}
        ...
        for idx, decoder_layer in enumerate(self.layers):
            layer_outputs = decoder_layer(
                ...
                output_attentions=output_attentions,
                use_cache=use_cache,
                extract_intermediate_representation=extract_intermediate_representation,
            )
            ...
            if extract_intermediate_representation:
                # Extract intermediate representations if they are included in the layer outputs
                if len(layer_outputs) > 3:
                    layer_intermediate_representations = layer_outputs[-1]
                    for key, value in layer_intermediate_representations.items():
                        if key in intermediate_representations_aggregated:
                            intermediate_representations_aggregated[key].append(value)
                        else:
                            intermediate_representations_aggregated[key] = [value]
        ...
        return RepBaseModel(
            ...
            intermediate_representations=intermediate_representations_aggregated if extract_intermediate_representation else None,
        )
...
class RepCausal(CausalLMOutputWithPast):
    def __init__(self, intermediate_representations=None, **kwargs):
        super().__init__(**kwargs)
        self.intermediate_representations = intermediate_representations
...
class CogAgentForCausalLM(CogAgentPreTrainedModel):
    ...
    def forward(
        ...
        if extract_intermediate_representation:
            intermediate_representations = outputs[-1]
        ...
        return RepCausal(
            ...
            intermediate_representations=outputs.intermediate_representations if extract_intermediate_representation else None,
        )
