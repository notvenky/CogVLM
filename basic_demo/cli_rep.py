...
from sat.model import AutoModel
from utils.utils import load_and_preprocess_image

def calculate_cosine_similarity(tensor1, tensor2):
    tensor1_flat = tensor1.view(-1)
    tensor2_flat = tensor2.view(-1)
    similarity = 1 - cosine(tensor1_flat.numpy(), tensor2_flat.numpy())
    return similarity
...
def main():
    ...
    parser.add_argument("--extract_representation", action="store_true", help="Extract image representation for comparison")
    parser.add_argument("--compare_images", action="store_true", help="Compare two images using their representations")
    ...
    # model = CogAgentModel(args)
    # model.eval()

    if args.compare_images:
        image_paths = []
        for i in range(2):
            image_path = input(f"Please enter the path for image {i+1}: ")
        ...
        for image_path in image_paths:
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    model.cuda()
                with torch.no_grad():
                    representation = model(processed_image.unsqueeze(0))
                representations.append(representation)
                print(f"Processed and extracted representation for {image_path}")
            except Exception as e:
                print(f"An error occurred while processing the image {image_path}: {e}")
                return

        similarity_score = calculate_cosine_similarity(representations[0].squeeze(), representations[1].squeeze())
        print(f"Similarity score between the images: {similarity_score:.4f}")
