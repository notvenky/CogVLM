...
class LangSAM():
    ...
    def predict_sam(self, image_pil, boxes):
        ...
        # Get the image embeddings/representation
        image_representation = self.sam.get_image_embedding()
        ...
        return masks.cpu(), image_representation.cpu()
    ...
    def predict(self, image_pil, text_prompt, box_threshold=0.3, text_threshold=0.25):
        ...
        if len(boxes) > 0:
            # import ipdb; ipdb.set_trace()
            masks, image_representation = self.predict_sam(image_pil, boxes)
            # image_representation = image_representation.view(1, 256, -1)
            masks = masks.squeeze(1)
